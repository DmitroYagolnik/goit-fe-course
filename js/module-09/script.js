...
/*
if(event.target === stopwatch.querySelector('.js-start')) 
Обращение к DOM самая медленная операция в этом приложении. 
Задержка в ее выполнении может быть минимум в 1000 раз больше чем любая другая. 
Для оптимизации работы следует проделывать операции по поиску элементов в DOM один раз 
и размещать это в какой -то переменной или свойстве объекта (прием называется кеширование).

Очень популярен такой синтаксис
function clickBtn({target}) {
вместо 
function clickBtn(event) {
если нет обращения именно к event, а все обращения только к target
*/